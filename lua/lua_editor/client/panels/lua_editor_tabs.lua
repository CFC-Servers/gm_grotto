...
AccessorFunc( buttonPanel, "m_pPropertySheet", "PropertySheet" ) -- Creates Get* & Set* accessors
AccessorFunc( buttonPanel, "Name", 			 "Name" )
Derma_Hook( buttonPanel, "Paint", "Paint", "Tab" ) -- Binds the "Paint" function to some built-in "Tab" paint algorithm.

function buttonPanel:Init()
    ...
    self.OnStartDragging = function( panel )
        panel:DoClick()
...
function buttonPanel:DoDoubleClick() -- Edit name!
...
function buttonPanel:DoMiddleClick()
    self:Close()
end

function buttonPanel:DoRightClick()
    self:DoClick()
...
function buttonPanel:SetName( new )
    local old = self.Name
...
Derma_Hook( tabsPanel, "Paint", "Paint", "PropertySheet" )
AccessorFunc( tabsPanel, "m_pActiveTab", "ActiveTab" 	)
AccessorFunc( tabsPanel, "m_iPadding",	 "Padding" 		)

function tabsPanel:Init()
    ...
    self.AddBtn.ApplySchemeSettings = function( panel )
        panel:SetTextInset( 10, 4 )

        local w = panel:GetContentSize()
        panel:SetSize( w + 10, 28 )

        DLabel.ApplySchemeSettings( panel )
    end

    self.AddBtn.UpdateColours = function( panel, skin )
    ...
    self.Editor.OnSessionAdded = function( _, name )
        self:AddTab( FormatName( name ) )
    end

    self.Editor.OnSessionRemoved = function( _, name )
        self:CloseTab( self:GetTabByName( name ) )
    end

    self.Editor.OnSessionChanged = function( _, name )
...
function tabsPanel:GetEditor()
    return self.Editor
end

function tabsPanel:OnRemove()
    if not self.Loaded then return end
    self:SaveTabsOrder()
    self:SaveActiveTab()
end

function tabsPanel:GetTabByName( name )
    for _, Tab in ipairs( self.tabScroller.Panels ) do
...
function tabsPanel:SwitchToName( name )
...
function tabsPanel:SetActiveTab( tab )
...
function tabsPanel:OnTabNameChanged( oldName, newName )
    local Editor = self:GetEditor()
...
function tabsPanel:LoadTabs()
    for _, name in ipairs( util.JSONToTable( self:GetCookie( "TabsOrder" ) or "" ) or {} ) do
...
function tabsPanel:SaveTabsOrder()
    local tabs = {}
...
function tabsPanel:OnTabsSaved()
end

function tabsPanel:SaveActiveTab()
...
function tabsPanel:PerformLayout()
...
function tabsPanel:CloseTab( tab )
